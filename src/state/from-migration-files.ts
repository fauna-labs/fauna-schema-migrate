...
import {
  filePathToDatabase,
  loadFqlSnippet,
  retrieveLastMigrationVersionAndPathsForMigrationBefore,
} from '../util/files'
...
export const getAllLastDatabases = async (before: string | null = null, ignoreChildDb = true) => {
...
export const getLastMigrationSnippets = async (
  atChildDbPath: string[] = [],
  before: string | null = null,
  ignoreChildDb = true
): Promise<LoadedResourcesAndLastMigration> => {
  ...
  const lastMigration = pathsAndFiles.length > 0 ? pathsAndFiles[pathsAndFiles.length - 1].migration : '0'

  for (let i = 0; i < pathsAndFiles.length; i++) {
    const pathAndfile = pathsAndFiles[i]
    pathAndfile.expressions = await Promise.all(
      pathAndfile.files.map(async (f) => {
